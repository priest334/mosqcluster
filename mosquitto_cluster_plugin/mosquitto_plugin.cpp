...
extern "C" {
	...
	int mosquitto_auth_plugin_init(void **user_data, struct mosquitto_opt *opts, int opt_count) {
		ClusterPluginContext* context = new ClusterPluginContext();
		if (context) {
	...
	int mosquitto_auth_plugin_cleanup(void *user_data, struct mosquitto_opt *opts, int opt_count) {
		ClusterPluginContext* context = reinterpret_cast<ClusterPluginContext*>(user_data);
		if (context) {
			context->Cleanup();
			delete context;
		}
	...
	int mosquitto_auth_acl_check(void *user_data, int access, struct mosquitto *client, const struct mosquitto_acl_msg *msg) {
		if (MOSQ_ACL_WRITE == access) {
			ClusterPluginContext* context = reinterpret_cast<ClusterPluginContext*>(user_data);
			context->MosqProxyPass(client, access, msg->topic, msg->payload, msg->payloadlen, msg->qos, msg->retain);
		}
